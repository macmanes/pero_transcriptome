...
for i in `cat list`; do grep --max-count=1 --no-group-separator -wA1 $i 3*fasta > om.$i.fa; done 
...
while [ $n -lt $total ]; do
	...
	then
		...
		if [ -f om.$n.fa ] ; #does the file exist?
		then
			...
			then
				...
				java -Xmx5000m -jar $MACSE -prog alignSequences -seq om.$n.fa -out_NT om.$n.aln & #just do it!        
				let n=n+1
			else
